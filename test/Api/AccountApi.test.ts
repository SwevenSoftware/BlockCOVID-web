...
describe("accountApi", () => {
	it("correct login", () => {
		...
		expect(
			accountApi.login({ username: "user", password: "password" })
	...
	it("correct logout", () => {
		...
		expect(mockedAxios.delete).lastCalledWith("/api/account/logout", config)
	})
	...
	it("correct modifyAccount", () => {
		...
		expect(
			accountApi.modifyAccount("adminToken", "/api/account", {
				username: "user",
	...
	it("correct createAccount", () => {
		...
		expect(
			accountApi.createAccount("adminToken", {
				username: "user",
				password: "password",
				authorities: ["USER"],
			})
	...
	it("correct deleteAccount", () => {
		...
		expect(
			accountApi.deleteAccount("adminToken", "/api/account/", {
				username: "user",
			})
		).resolves.toEqual(axiosResponse)
		expect(mockedAxios.delete).lastCalledWith("/api/account/user", config)
	})
	...
	it("correct getAccounts", () => {
		...
		expect(accountApi.getAccounts("adminToken")).resolves.toEqual(
			axiosResponse
		)
		expect(mockedAxios.get).lastCalledWith("/api/users", config)
	})
})
