...
describe('Account Component', function() {
    ...
    it('should render the proper number of account icons', async function() {
        const wrapper = mount(<Provider store={store}><AccountComponent /></Provider>)
        expect(wrapper.find(AccountComponent).find(ListItemIcon).find(PeopleIcon).length).toBe(1);
    });

    it('should render the proper number of user icons', async function() {
        const wrapper = mount(<Provider store={store}><AccountComponent /></Provider>)
        expect(wrapper.find(AccountComponent).find(ListItemIcon).find(WorkIcon).length).toBe(1);
    });

    it('should render the proper number of admin icons', async function() {
        const wrapper = mount(<Provider store={store}><AccountComponent /></Provider>)
        expect(wrapper.find(AccountComponent).find(ListItemIcon).find(SecurityIcon).length).toBe(1);
    });

    it('should render the proper number of cleaner icons', async function() {
        const wrapper = mount(<Provider store={store}><AccountComponent /></Provider>)
        expect(wrapper.find(AccountComponent).find(ListItemIcon).find(BathtubIcon).length).toBe(1);
    });
