...
class ReportsComponent extends Component<ReportsProps, ReportsState> {
      ...
      <div className="marginAccounts">
        <ThemeProvider theme={theme}>
          <div className="addRoomsButton">
            <FormControl>
              <InputLabel shrink id="order-by-label">Ordina per</InputLabel>
              <Select
                labelId="ordina-per"
                id="order-by-select"
                value={this.state.orderBy}
                onChange={ (e) => {
                  this.handleChangeSelectedOrderBy(e.target.value as string)
                }}
              >
                <MenuItem value="nome">Nome report</MenuItem>
                <MenuItem value="creazione">Data creazione</MenuItem>
                <MenuItem value="registrazione">Data registrazione</MenuItem>
              </Select>
            </FormControl>
          </div>
          <div>
            {this.props.state.reports.error ? this.props.state.reports.error : ""}
  ...
  sort(a: ReportInformation, b: ReportInformation) {
    switch(this.state.orderBy) {
      ...
      case "registrazione":
        return (a.registrationDate < b.registrationDate ? 1 : b.registrationDate < a.registrationDate ? -1 : 0)
    }
  }
  ...
  private popolate(): Array<JSX.Element> {
    ...
    if (this.props.state.reports.reports) {
      this.props.state.reports.reports
        .sort((a: ReportInformation, b: ReportInformation) => this.sort(a, b))
