...
class ModifyAccountComponent extends Component<
	ModifyAccountProps,
	ModifyAccountStates
> {
	constructor(props) {
							...
							<div className="centralModal">
								...
								<div className="addField">
									<TextField
										...
										helperText={
											this.state.lengthPasswordError
												? ERROR_LENGTH_PASSWORD
												: ""
										}
	...
	private setButton(
		...
		authorities: boolean[] = this.state.authorities
	): void {
	...
	private setCheckboxes() {
		this.setState({
			authorities: {
				checkedAdmin:
					this.props.data.user.authorities.includes("ADMIN"),
				checkedUser: this.props.data.user.authorities.includes("USER"),
				checkedCleaner:
					this.props.data.user.authorities.includes("CLEANER"),
			},
		})
	}
	...
	private handleChangeAuthorities(
		...
		switch (event.target.name) {
			case "checkedAdmin":
				this.authInputControl([
					event.target.checked,
					this.state.authorities.checkedUser,
					this.state.authorities.checkedCleaner,
				])
				break
			case "checkedUser":
				this.authInputControl([
					this.state.authorities.checkedAdmin,
					event.target.checked,
					this.state.authorities.checkedCleaner,
				])
				break
	...
	private handleConfirm(): void {
		...
		if (!flagErr) {
			...
			this.props.dispatch.modifyAccount(this.props.data.user.link, {
