...
class DotGrid extends Component<DotGridProps> {
	...
	public setSize(width: number, height: number): boolean {
		this.canvasRef.current?.setAttribute(
			"width",
			((width + 1) * this.gridSettings.dim).toString()
		)
		this.canvasRef.current?.setAttribute(
			"height",
			((height + 1) * this.gridSettings.dim).toString()
		)
	...
	checkBox(e: React.MouseEvent<HTMLCanvasElement, MouseEvent>): void {
		const boundRect: DOMRect | undefined =
		...
		if (this.grid.isFree(pointPos)) {
			...
			for (var i in this.removedDesks) {
				if (
					this.removedDesks[i].pos.x == pointPos.x &&
			...
			if (!reinserted) this.grid.addDesk(pointPos)
		} else {
			const selectedDesk = this.grid.getDesk(pointPos)
			if (selectedDesk && !selectedDesk.inUse) {
	...
	checkDistance(
		...
		if (mouse / dim >= 1 - step / dim || mouse < point) return false
	...
	updateCanvas(): void {
		...
		for (var x: number = radius; x <= canvas.width - radius; x += dim) {
			for (
				...
				if (
					this.checkDistance(
						this.mousePos.x - canvasRect.x,
						x,
						dim,
						canvasRect.width
					) &&
					this.checkDistance(
						this.mousePos.y - canvasRect.y,
						y,
						dim,
						canvasRect.height
					)
		...
		for (var pos of positions) {
			...
			if (desk != undefined) this.drawDesk(ctx, desk, radius * 4)
		}
	}
